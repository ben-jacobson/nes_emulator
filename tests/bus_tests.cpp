...
TEST_CASE_METHOD(emulator_test_fixtures, "bus - Test for correct registration order") {
    ...
    REQUIRE(test_bus.devices_connected_to_bus[1]._address_range_start == 0x4000);
...
TEST_CASE_METHOD(emulator_test_fixtures, "bus - Test get_index_of_connected_device ordering") {
    ...
    REQUIRE(test_bus.devices_connected_to_bus[test_bus.get_index_of_connected_device(0x4000 + 1)]._address_range_start == 0x4000);
    ...
    REQUIRE(test_bus.get_index_of_connected_device(0x4000 + 1) == 1);
