...
TEST_CASE_METHOD(emulator_test_fixtures, "bus - Test for correct registration order") {
    ...
    REQUIRE(test_bus.devices_connected_to_bus[1]._address_range_start == RAM_MIRRORA_SPACE_START);
    REQUIRE(test_bus.devices_connected_to_bus[2]._address_range_start == CART_ADDRESS_SPACE_START);
...
TEST_CASE_METHOD(emulator_test_fixtures, "bus - Test get_index_of_connected_device ordering") {
    ...
    REQUIRE(test_bus.devices_connected_to_bus[test_bus.get_index_of_connected_device(RAM_MIRRORA_SPACE_START + 1)]._address_range_start == RAM_MIRRORA_SPACE_START);
    ...
    REQUIRE(test_bus.get_index_of_connected_device(RAM_MIRRORA_SPACE_START + 1) == 1);
    REQUIRE(test_bus.get_index_of_connected_device(CART_ADDRESS_SPACE_START + 1) == 2);    
